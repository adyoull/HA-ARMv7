...
Status: **working.** 2026.9.0 builds, boots, and restores a 2025.11.3 backup. In
...
docker run -d --name ha-test --platform linux/arm/v7 \
  -p 8123:8123 -v /tmp/hatest:/config ha-armv7:2026.9.0
...
backup restores cleanly into 2026.9.0 here, the Pi migration is de-risked.
...
```bash
# Mac
docker save ha-armv7:2026.9.0 | gzip -1 > ha-armv7-2026.9.0.tar.gz
scp ha-armv7-2026.9.0.tar.gz pi@192.168.0.10:~/

# Pi
gunzip -c ha-armv7-2026.9.0.tar.gz | docker load
docker compose up -d
```
...
```bash
docker tag ha-armv7:2026.9.0 ghcr.io/<user>/ha-armv7:2026.9.0
docker push ghcr.io/<user>/ha-armv7:2026.9.0   # needs a classic PAT with write:packages
```
