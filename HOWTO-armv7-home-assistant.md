...
can still run a *current* release (2026.9.0 and beyond) on a Raspberry Pi 2/3 or
...
services:
  homeassistant:
    image: ghcr.io/adyoull/ha-armv7:2026.9.0-r1
...
docker run --rm --platform linux/arm64 --entrypoint python \
  ...
  -m pip freeze > official-2026.9.0.txt
```
...
ARG CONSTRAINTS=official-2026.9.0.txt
...
scp ha-armv7-2026.9.0.tar.gz pi@<pi-ip>:~/
...
2025.11.3 backup restores cleanly into 2026.9.0 there, the migration is de-risked
