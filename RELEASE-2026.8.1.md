# Home Assistant 2026.8.1 for ARMv7 — `2026.8.1-r1`

Unofficial 32-bit ARM (armv7) build of Home Assistant **2026.8.1**, for Raspberry
...
Also tagged `2026.8.1` and `latest`. Pin to **`2026.8.1-r1`** for a stable,
unchanging reference — `latest` will move when a newer HA build is published.

| Tag | Meaning |
|---|---|
| `2026.8.1-r1` | This exact image. Immutable — safe to pin. |
| `2026.8.1` | Rolling pointer for HA 2026.8.1 (currently = r1). |
| `latest` | Newest build overall. Will change under you. |

## What changed since `2026.8.0-r1`

**Home Assistant upstream bumped 2026.8.0 → 2026.8.1** (patch release). This image
rebuilds against it, with the official 2026.8.1 dependency pins. Still Python
3.14; no base-image change.

No changes to the build recipe. Everything carried over from 2026.8.0-r1:

- Base `python:3.14-slim-trixie`, FFmpeg 8 built from source for PyAV (cached
  above the HA install, so it did not recompile for this bump)
...
docker run --rm --platform linux/arm64 --entrypoint python \
  ghcr.io/home-assistant/home-assistant:2026.8.1 -m pip freeze > official-2026.8.1.txt
```
...
Anyone on 2026.8.0-r1 who wants the upstream patch fixes. Routine patch — no
...
docker run --rm --platform linux/arm/v7 --entrypoint python \
  ghcr.io/adyoull/ha-armv7:2026.8.1-r1 \
...
Expected: `2026.8.1 | pyatv 0.18.0 | pyotp 2.9.0`
