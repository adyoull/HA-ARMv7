# Home Assistant 2026.9.2 for ARMv7 — `2026.9.2-r1`

Unofficial 32-bit ARM (armv7) build of Home Assistant **2026.9.2**, for Raspberry
...
Also tagged `2026.9.2` and `latest`. Pin to **`2026.9.2-r1`** for a stable,
unchanging reference — `latest` will move when a newer HA build is published.

| Tag | Meaning |
|---|---|
| `2026.9.2-r1` | This exact image. Immutable — safe to pin. |
| `2026.9.2` | Rolling pointer for HA 2026.9.2 (currently = r1). |
| `latest` | Newest build overall. Will change under you. |

## What changed since `2026.9.1-r1`

**Home Assistant upstream bumped 2026.9.1 → 2026.9.2** (patch release). This image
rebuilds against it, with the official 2026.9.2 dependency pins. Still Python 3.14.

No changes to the build recipe. Everything carried over from 2026.9.1-r1:

- Base + app image split (FFmpeg 8 and Cython built once, in the base)
- PyAV source patch for Cython 3.1.7+ (`patch_av.sh`)
...
docker run --rm --platform linux/arm64 --entrypoint python \
  ghcr.io/home-assistant/home-assistant:2026.9.2 -m pip freeze > official-2026.9.2.txt
```
...
Anyone on 2026.9.1-r1 who wants the upstream patch fixes. Routine patch — no
...
docker run --rm --platform linux/arm/v7 --entrypoint python \
  ghcr.io/adyoull/ha-armv7:2026.9.2-r1 \
...
Expected (confirm `av` matches the 2026.9.2 pin): `2026.9.2 | av 17.0.1 | pyatv
0.18.0 | pyotp 2.9.0 | bleak-smlight 1.1.0`
